...
func GetLeaderboardEntry(ctx context.Context, email string, isDemo bool) (*LeaderboardEntry, error) {
	...
	err := Pool.QueryRow(ctx, `
		...
		vp_data AS (
			SELECT user_email, COALESCE(SUM(points), 0) AS vantic_points
			FROM vantic_points_ledger WHERE is_demo = $2
			GROUP BY user_email
		),
		pnl_data AS (
			SELECT user_email, COALESCE(SUM(realized_pnl), 0) AS total_pnl
			FROM positions WHERE is_demo = $2 AND status = 'SETTLED'
			GROUP BY user_email
		),
		trade_data AS (
			SELECT user_email, COUNT(*) AS trade_count
			FROM positions WHERE is_demo = $2
			GROUP BY user_email
		),
		deposit_data AS (
			SELECT user_email, COALESCE(SUM(amount), 0) AS total_deposits
			FROM transactions WHERE type IN ('deposit', 'faucet')
			GROUP BY user_email
		),
		withdrawal_data AS (
			SELECT user_email, COALESCE(SUM(amount), 0) AS total_withdrawals
			FROM transactions WHERE type IN ('withdrawal', 'asset_withdrawal')
			GROUP BY user_email
		),
		raw AS (
			SELECT
		...
		ordered AS (
			SELECT *, ROW_NUMBER() OVER (ORDER BY (vantic_points + activity_score) DESC)::int AS rank
			FROM with_score
		)
		SELECT email, username, profile_image_url, pnl, trades, deposits, withdrawals, vantic_points, activity_score, rank
		FROM ordered
		WHERE email = $1
	`, email, isDemo).Scan(
		&e.Email, &e.Username, &e.ProfileImageURL,
		&e.PnL, &e.Trades, &e.Deposits, &e.Withdrawals,
		&e.VanticPoints, &e.ActivityScore, &e.Rank,
	)
	if err != nil {
		return nil, err
	}
